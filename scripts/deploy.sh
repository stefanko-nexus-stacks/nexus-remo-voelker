...
SSH_HOST="ssh-remo-voelker.nona.company"
...
KESTRA_URL=https://kestra-remo-voelker.nona.company
EOF
...
CB_SERVER_URL=https://cloudbeaver-remo-voelker.nona.company
...
REDIRECT_URL=https://hoppscotch-remo-voelker.nona.company
WHITELISTED_ORIGINS=https://hoppscotch-remo-voelker.nona.company
VITE_BASE_URL=https://hoppscotch-remo-voelker.nona.company
VITE_SHORTCODE_BASE_URL=https://hoppscotch-remo-voelker.nona.company
VITE_ADMIN_URL=https://hoppscotch-remo-voelker.nona.company/admin
VITE_BACKEND_GQL_URL=https://hoppscotch-remo-voelker.nona.company/backend/graphql
VITE_BACKEND_WS_URL=wss://hoppscotch-remo-voelker.nona.company/backend/graphql
VITE_BACKEND_API_URL=https://hoppscotch-remo-voelker.nona.company/backend/v1
...
PREFECT_UI_API_URL=https://prefect-remo-voelker.nona.company/api
EOF
...
LAKEFS_GATEWAYS_S3_DOMAIN_NAME=s3.lakefs-remo-voelker.nona.company
...
APPSMITH_CUSTOM_DOMAIN=https://appsmith-remo-voelker.nona.company
EOF
...
NC_PUBLIC_URL=https://nocodb-remo-voelker.nona.company
...
EOF
    ...
    if [ -n "$INFISICAL_TOKEN" ] && [ -n "$PROJECT_ID" ]; then
        ...
        build_folder "redpanda" \
            ...
            "REDPANDA_KAFKA_PUBLIC_URL" "redpanda-kafka-remo-voelker.nona.company:9092" \
            "REDPANDA_SCHEMA_REGISTRY_PUBLIC_URL" "redpanda-schema-registry-remo-voelker.nona.company:18081" \
            "REDPANDA_ADMIN_PUBLIC_URL" "redpanda-admin-remo-voelker.nona.company:9644" \
            "REDPANDA_CONNECT_PUBLIC_URL" "redpanda-connect-api-remo-voelker.nona.company:4195"
        ...
        build_folder "gitea" \
            ...
            "GITEA_REPO_URL" "https://git-remo-voelker.nona.company/${GITEA_REPO_OWNER:-$ADMIN_USERNAME}/${REPO_NAME:-nexus-${DOMAIN//./-}-gitea}.git" \
            "GITEA_DB_PASSWORD" "$GITEA_DB_PASS"
    ...
    cron: \"*/15 * * * *\"'" >/dev/null 2>&1 || true
            ...
            if echo "$ENABLED_SERVICES" | grep -qw "woodpecker" && [ -n "$WOODPECKER_AGENT_SECRET" ]; then
                ...
                OAUTH_RESULT=$(ssh nexus "curl -s -X POST 'http://localhost:3200/api/v1/user/applications/oauth2' \
                    ...
                    -d '{
                        \"name\": \"Woodpecker CI\",
                        \"redirect_uris\": [\"https://woodpecker-remo-voelker.nona.company/authorize\"],
...
if echo "$ENABLED_SERVICES" | grep -qw "wikijs" && [ -n "$WIKIJS_ADMIN_PASS" ]; then
    (
        ...
        SETUP_PAYLOAD=$(jq -n \
            ...
            --arg url "https://wiki-remo-voelker.nona.company" \
