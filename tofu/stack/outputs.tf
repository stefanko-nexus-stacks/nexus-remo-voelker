...
output "ssh_command" {
  ...
  value       = "cloudflared access ssh --hostname ssh-remo-voelker.nona.company"
}
...
output "ssh_config" {
  ...
  value       = <<-EOT
    Host nexus
      HostName ssh-remo-voelker.nona.company
      User root
...
output "service_urls" {
  ...
  value = {
    ...
    key => "https://${service.subdomain}-remo-voelker.nona.company"
  }
}
...
output "secrets" {
  ...
  value = {
    ...
    redpanda_schema_registry_public_url = "http://redpanda-schema-registry-remo-voelker.nona.company:18081"
